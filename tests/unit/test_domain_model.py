...
class TestBook:
    ...
    def test_attribute_setters(self):
        ...
        book.rating = 4.12
        assert book.rating == 4.12
        temp_tags = ['Horror', 'Mystery', 'Action']
        book.tags = temp_tags
        assert set(temp_tags) == {'Horror', 'Mystery', 'Action'}
        book.image_url = "https://images.gr-assets.com/books/1453061084m/27036537.jpg"
        assert book.image_url == "https://images.gr-assets.com/books/1453061084m/27036537.jpg"
...
class TestReview:
    ...
    def test_construction(self):
        ...
        rating = 4
        review = Review(None,book, review_text, rating)
    ...
    def test_invalid_parameters(self):
        ...
        with pytest.raises(ValueError):
            review = Review(None,book, review_text, -1)

        with pytest.raises(ValueError):
            review = Review(None,book, review_text, 6)

    def test_set_of_reviews(self):
        book1 = Book(2675376, "Harry Potter")
        book2 = Book(874658, "Lord of the Rings")
        review1 = Review(None,book1, "I liked this book", 4)
        review2 = Review(None,book2, "This book was ok", 3)
        review3 = Review(None,book1, "This book was exceptional", 5)
    ...
    def test_wrong_book_object(self):
        ...
        review = Review(None, publisher, "I liked this book", 4)
...
def read_books_and_authors():
    ...
    return repo.repo_instance.books


@pytest.fixture
def create_json_reader():
    books_file_name = 'comic_books_excerpt.json'
    authors_file_name = 'book_authors_excerpt.json'
    reviews_file_name = "reviews.json"
    repo.repo_instance = MemoryRepository()
    ...
    reader = BooksJSONReader(path_to_books_file, path_to_authors_file, path_to_reviews_file)
    reader.read_json_files(repo.repo_instance)
    #print(repo.repo_instance.books)
...
class TestSearchMethod:
    ...
    def test_search_by_tag(self, create_json_reader):
        create_json_reader
        found = repo.repo_instance.get_books_by_tag("slice-of-life")
        # print(f"found {found[0]}")
        assert str(found[0]) == "<Book \u7e8c\uff0e\u661f\u5b88\u72ac, book id = 18711343>"#repo.repo_instance.books[17]

    def test_search_by_author(self, create_json_reader):
        create_json_reader
        author_to_find = repo.repo_instance.books[0].authors[0]
        found = repo.repo_instance.get_books_by_author(author_to_find)
        assert found[0].authors[0] == author_to_find

    def test_search_by_date_range(self, create_json_reader):
        create_json_reader
        found = repo.repo_instance.get_books_by_date_range(2016, 2016)
        # print(f"found {found[0]}")
        assert found[0] == repo.repo_instance.books[1]

    def test_search_by_title(self, create_json_reader):
        create_json_reader
        found = repo.repo_instance.get_books_by_title("Holmes")
        print("there are ", len(found))
        assert str(found[0]) == "<Book Sherlock Holmes: Year One, book id = 11827783>"
        pass

    def test_review_search_by_book(self, create_books_150_books):
        create_books_150_books
        found_items = []
        for review in repo.repo_instance.reviews:
            print(repo.repo_instance.get_book(23012586))
            if str(review.book) == str(repo.repo_instance.get_book(23012586)):
                found_items.append(review)
        print(found_items)
